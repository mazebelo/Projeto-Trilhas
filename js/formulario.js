...
let cepDataReceived = false; // verifica e controla se os dados do CEP foram recebidos
...
form.addEventListener("submit", (event) => {
    ...
    setTimeout(() => {
        if (cepProcessado) {
            ...
            console.log("CEP não processado ou inválido. Salvamento cancelado.");
        }
    }, 1000);

    console.log("Validações iniciais concluídas, aguardando dados do CEP...")
});

function salvarInformComCep() {
    ...
    const trilha = Array.from(document.getElementsByName('trilha')).find(radio => radio.checked)?.value || "";
    const termos = document.getElementById('termos').checked ? "Termos aceitos" : "Termos não aceitos";
    const arquivo1 = document.getElementById('arquivo1').files.length > 0 ? "Documento inserido:" + document.getElementById('arquivo1').files[0].name : "";
    const arquivo2 = document.getElementById('arquivo2').files.length > 0 ? "Documento inserido:" + document.getElementById('arquivo2').files[0].name : "";
    ...
    const dadosParaSalvar = {
        ...
        rua: rua,
        numero: numero,
        cidade: cidadePreenchidaPeloCep,
        uf: ufPreenchidaPeloCep,
        trilha: trilha,
        termos: termos,
        arquivo1: arquivo1,
        arquivo2: arquivo2,
    };
    salvarNoLocalStorage(dadosParaSalvar);
}
...
let cidadePreenchidaPeloCep = "";
let ufPreenchidaPeloCep = "";
let cepProcessado = false;
...
function meu_callback(conteudo) {
    ...
    if (!("erro" in conteudo)) {
        ...
        cidadePreenchidaPeloCep = conteudo.localidade;
        ufPreenchidaPeloCep = conteudo.uf;
        cepProcessado = true; // Indica que o CEP foi processado e os dados preenchidos
        console.log("Dados do CEP recebidos e processados.");
